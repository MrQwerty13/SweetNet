...
восстановление через Docker-скрипты и HTTPS на реальном домене пока **не проверены**.
Реальная native-проверка PostgreSQL dump/restore и каталога uploads выполнена
через `tests/verify-backup.mjs`; она не подменяет контейнерную проверку.
Синтаксическая проверка Compose не заменяет эти проверки. Закреплённые теги
Go, Node, PostgreSQL и Caddy найдены через `docker manifest inspect`. Точные результаты
...
## Проверка native-восстановления без Docker

На PostgreSQL 17.11 проверен `node tests/verify-backup.mjs` с выделенной
`TEST_DATABASE_URL` (база с суффиксом `_test`). Скрипт создаёт две новые случайно
названные базы, применяет миграции, создаёт владельца и друга, публикует фото,
останавливает процесс на время `pg_dump` и tar, проверяет manifest/checksum,
восстанавливает вторую базу и uploads, затем запускает новый процесс.
Подтверждены: повторный вход обоих аккаунтов через исходную/восстановленную БД,
пост, побайтное совпадение фото, запрет гостю, запрет другу удалять чужой пост,
сохранение использованного приглашения, перезапуск с прежней сессией, блокировка
online-сверки. Только созданные этим тестом базы и временные файлы удаляются.

`node tests/verify-compose.mjs` повторяет основной путь непосредственно через
Dockerfile, Compose, `scripts/backup.sh` и `scripts/restore.sh`. Он требует Docker
Engine, доступен в CI и пока не имеет локального результата. Скрипт создаёт
отдельные случайные тестовые проекты и удаляет только их контейнеры/volumes.

## Параметры паролей и ограничение ресурсов

Argon2id: память 19 MiB, 2 итерации, parallelism 1, соль 16 байт, ключ 32 байта;
значения зафиксированы рядом с `HashPassword`. Это соответствует минимальной
конфигурации [OWASP Password Storage](https://cheatsheetseries.owasp.org/cheatsheets/Password_Storage_Cheat_Sheet.html).
Замер `go test -run '^$' -bench BenchmarkHashPassword -benchtime=3x ./internal/auth`
на Apple M3: примерно 25.7 ms на хеш. Перед production повторите замер на VPS;
это измерение не является оценкой серверной пропускной способности.
Не более двух HTTP-запросов одновременно выполняют операции с паролями;
дополнительно работают лимиты по времени/IP/username. Изображения обрабатываются
максимум двумя запросами одновременно. Память PostgreSQL, Go и декодированных
изображений нужно учитывать при выборе размера VPS.
